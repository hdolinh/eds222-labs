...
# rootdir <- ("~/Dropbox/Teaching/UCSB/EDS_222/EDS222_data/") this is commented out because I have an .RProj set up
...
#source(file.path(rootdir,"labs","_common.R"))  this is an alternative way to tell R where the _common.R script is

source(here::here("01-lab", "_common.R")) # I am using here to tell R where I keep _common.R 
...
```{r}
loans_clean <- loans_full_schema %>% 
  mutate(
    homeownership = tolower(homeownership),
    homeownership = fct_relevel(homeownership, "rent", "mortgage", "own"), # use levels() to view order
    application_type = fct_relevel(application_type, "joint", "individual")
  )

# side note: use str(data.frame) to see all class types and more. This shows you the structure of the data frame.
```
...
# bar plots are histograms for categorical data

# bar plot showing number of homeownership types
p_count <- ggplot(loans_clean, aes(x = homeownership)) +
  geom_bar(fill = IMSCOL["green", "full"]) + # type IMSCOL into the console to see all available colors
  labs(x = "Homeownership", y = "Count")

p_count

# bar polot showing proportion of loans in each homeownership type
p_proportion <- loans_clean %>% 
  count(homeownership) %>% # count provides n
  mutate(proportion = n / sum(n)) %>% # created new column proportion to find proportion
  ggplot(aes(x = homeownership, y = proportion)) +
  geom_col(fill = IMSCOL["pink", "f1"]) +
  labs(x = "Homeownership", y = "Proportion")

p_proportion

p_count + p_proportion # use patchwork when you want to do more customization with multiple plots 
```
...
# feel free to go back and add IMSCOL colors, labs, and theme later
p_stacked <- ggplot(loans_clean, aes(x = homeownership, fill = application_type)) +
  geom_bar() +
...
p_dodged <- ggplot(loans_clean, aes(x = homeownership, fill = application_type)) +
  geom_bar(position = "dodge")

p_dodged

p_standarized <- ggplot(loans_clean, aes(x = homeownership, fill = application_type)) +
